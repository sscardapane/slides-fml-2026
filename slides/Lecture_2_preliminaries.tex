...
\begin{frame}{Prodotto scalare e cosine similarity}
	...
	%
	\begin{equation*}
		\lVert \vc{x} - \vc{y} \rVert^2 = \langle \vc{x}, \vc{x} \rangle + \langle \vc{y}, \vc{y} \rangle - 2 \langle \vc{x}, \vc{y} \rangle \,.
	\end{equation*}
...
\begin{frame}{Moltiplicazione tra matrici}
	...
	\end{tcolorbox}
	%
	In questa forma, ogni colonna di $\vc{Z}$ è il prodotto matrice-vettore tra $\vc{X}$ ed una colonna di $\vc{Y}$, ovvero $\vc{Z}_{:,j} = \vc{X} \vc{Y}_{:,j}$.
\end{frame}
...
\begin{frame}{ML per una distribuzione categorica}
	...
	%
	\begin{equation}
		p_i = \frac{\sum_{j=1}^n \idx{\mathbf{x}_j}{i}}{n}
	\end{equation}
	%
